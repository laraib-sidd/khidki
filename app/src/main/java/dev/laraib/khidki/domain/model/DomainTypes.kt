...
data class AuthorizationSession(
    ...
    val destinationSnapshots: List<DestinationSnapshot> = emptyList(),
    val untilStop: Boolean = false,
) {
    ...
    fun activeDestinations(): List<DestinationSnapshot> =
        if (destinationSnapshots.isNotEmpty()) {
            destinationSnapshots
        } else if (origin == SessionOrigin.TIMED) {
            listOf(
                DestinationSnapshot(
                    id = "legacy",
                    name = label,
    ...
    fun activeConfiguration(): Configuration {
        ...
        val snapshotWindow =
            when (origin) {
                SessionOrigin.TIMED ->
                    windowSeconds.coerceIn(
                        CredentialPolicy.MIN_WINDOW_SECONDS,
                        CredentialPolicy.MAX_WINDOW_SECONDS,
                    ).takeIf { it > 0 } ?: CredentialPolicy.DEFAULT_WINDOW_SECONDS
                SessionOrigin.REQUEST -> windowSeconds
            }
        return Configuration(
            ...
            credentialPolicy = CredentialPolicy(windowSeconds = snapshotWindow),
            enabled = true,
            windowSeconds = snapshotWindow,
        )
    }
}
