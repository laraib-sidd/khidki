...
|---|---|
| **Git tag** | `v1.0.2` |
| **APK name** | `khidki-1.0.2-b3-<sha>.apk` |
| **versionName / versionCode** | Baked in `app/build.gradle.kts` (`1.0.2` / `3`) |
...
```bash
git tag -a v1.0.2 -m "Khidki 1.0.2"
...
2. Download `khidki-1.0.2-*.apk` (package `dev.laraib.khidki`)
...
```bash
export KHIDKI_VERSION_NAME=1.0.2
